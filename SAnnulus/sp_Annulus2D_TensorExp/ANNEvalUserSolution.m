...
%     end
    ...
    % Example 4: 
    %     
    %     zMat_sol(:, i) = S-Curve of degree k.;
    prob_order = 9;
    ...
    for i = 1 : nth
        ...
        sth = cos(theta);
        if deriv_l == 0   %   Evaluation of original function
            ueval(i, 1) = sth * spEvalPolynomialLe(alphas, Nalpha, left, 1);
        elseif deriv_l == 1   %   Evaluation of derivative
            ueval(i, 1) = sth * spEvalPolynomialLeDeriv1(alphas, Nalpha, left, 1);
        elseif deriv_l == 2   %   Evaluation of 2nd derivative
            ueval(i, 1) = sth * spEvalPolynomialLeDeriv2(alphas, Nalpha, left, 1);
        else
        end
    end
    ...
    for i = 1 : nth
        ...
        sth = cos(theta);
        if deriv_r == 0   %   Evaluation of original function
            ueval(i, 2) = sth * spEvalPolynomialLe(alphas, Nalpha, right, 1);
        elseif deriv_r == 1   %   Evaluation of derivative
            ueval(i, 2) = sth * spEvalPolynomialLeDeriv1(alphas, Nalpha, right, 1);
        elseif deriv_r == 2   %   Evaluation of 2nd derivative
            ueval(i, 2) = sth * spEvalPolynomialLeDeriv2(alphas, Nalpha, right, 1);
        else
        end
    end
