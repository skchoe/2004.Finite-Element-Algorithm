...
%
function max_error = sp1DSolvePoisson(BdyType, left, right, dxvec, pvec, vBL, vBR)
    ...
    for el = 1:N
        ...
        if BdyType == 'DD'
            [uvec, fzs, vBL, vBR] = spEvalUserFunctionsDD(left, right, xiz, size(xiz,1), vBL, vBR);
        elseif BdyType == 'DN'
            [uvec, fzs, vBL, vBR] = spEvalUserFunctionsDN(left, right, xiz, size(xiz,1), vBL, vBR);
        else 
        end

        % Global y vector definition
        yvec_sol = [yvec_sol;uvec];
    ...
    [Lg, Fg] = ApplyBdy2Matrix(BdyType, Ndof, Pn, Lg, Fg, vBL, vBR);
    ...
    figno = 1;
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %max_error = spPlotResultDD(numsampleintervals, left, right, alpha, beta, dxvec, pvec, bdleft, bdright, Lg, ug_hat, map, figno);
    figure(figno);
...
function [Lg, Fg] = ApplyBdy2Matrix(BdyType, Ndof, Pn, Lg, Fg, vBL, vBR)
...
function basisTable = setup_basisTable(alpha, beta, N, idxQpts, pvec, zwTable, basisTable)
    for ei = 1:N
        ...
        for ip = 0:p_i
            for iz = 1:z_i
                z = zwTable(iz, 1, ei);
                basisTable(iz, ip+1, ei) = ModifiedJacobiPoly(ip, z, alpha, beta);
            end
        end
    end
