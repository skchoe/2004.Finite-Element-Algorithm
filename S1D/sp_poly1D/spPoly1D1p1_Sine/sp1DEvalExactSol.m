...
function [xvec, yvec_sol] = sp1DEvalExactSol(BdyType, left, right, xvec, vBL, vBR);
  ...
  if BdyType == 'DN'
    ...
    Nvec = size(xvec, 1); 
    yvec_sol = zeros(Nvec, 1);
    yvec_sol = -1/pi2 * sin(pi * xvec) + C * xvec + D;
  ...
  elseif BdyType == 'DD'
    ...
    tL = -1/(pi2) * sin(pi*left);
    tR = -1/(pi2) * sin(pi*right);
    dt = tR - tR;
...
%   %------------------------------------------------------------------------
%   % Given K>0, f = K*(K-1)x^(K-2) -> u = x^K, DB(0) = 1, NB(1) = K  - Need to be maintained
%   K = 21;
...
%   %------------------------------------------------------------------------
%   % S-Curve (1) with Monomial Basis, 2nd derivative, Neumann, Dirichlet BDY's - Need to be maintained
%   prob_order = 9;
...
%     %------------------------------------------------------------------------
%     % S-Curve (2) with Legendre Basis, 2nd derivative, Neumann, Dirichlet BDY's
%     prob_order = 9;
