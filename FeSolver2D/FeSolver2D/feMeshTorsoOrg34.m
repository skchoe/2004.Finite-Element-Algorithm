...
% Number of Interior nodes : (545) {52,
% ...,197,258,...579,581,...,649,651,...658}  
...
function [NdMat, Elm, BdyD, BdyN, IntNd, BdyDP, NdcondF] = ...
  ...
  NdmF = load(file_nodes);      %  Nnd = 659;      % Number of
                                %  Total Nodes 16/48/180 
  ...
  %switch the triangle connectivity orientation
  for i = 1:NelF
    n2 = ElmF(i, 2);
    ElmF(i, 2) = ElmF(i,3);
    ElmF(i, 3) = n2;
  ...
  NdMat = sparse(Nnd, 2);                    % [X, Y] Coordinate
                                             % set of all nodes 
...
% insert two points into the exterior 
  spIdx1 = 580;
  spIdx2 = 650;
  nBdyN = [BdyN(1:12); spIdx2; BdyN(13:21); spIdx1; BdyN(22:end-2)];
  BdyN = nBdyN;
