...
Egm = zeros(NelmF, 3);
Egm(:,:) = ElcondEl(:,:,1);    % This shows Internal Edge/Boundary
                               % Edge, doesn't show Bd-D/Bd-N 
...
for in = 1:NndF    % copy the content of NdEltCell into NdqElltCell
  ...
  %This means the node is on the geometric node
  NdqEltCell{newIn, 3} = zeros(size(NdEltCell{in, 1}));   
end
for je = 1:NelmF
  for ke = 1:3
	...
	NdqEltCell{nod, 2} = [NdqEltCell{nod, 2}; ElcondEl(je, ...
						  ke, 2)]; 
	NdqEltCell{nod, 3} = [NdqEltCell{nod, 3}; 0.5];
      end
    end
