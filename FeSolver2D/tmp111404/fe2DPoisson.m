...
function [Ag, Fg, Fg_BC, Fg_K, SolIn, SolBd, BdyDP, BdyDP_2, Ndm, ...
	      IntNd, BdyN, BdyD, Elm, phiXY, Wpq, JacVec, VecXYm] ...
        = fe2DPoisson(gNdMat, gElm, gBdyD, gBdyN, gIntNd, gBdyDP, ...
          gElcond, bcVar, bsOdr, degMax, isLaplace, ...
          infoFlag, itrSize, exeTag) 

tol = 1e-12; %tolerance for the preconditioned conjugate gradient solver 

disp(['iteration of solver = ' num2str(exeTag)]);
...
drFlag = 0;
[Ndm, Elm, BdyN, BdyD, IntNd, BdyDP, Elcond, Map2New] = ...
    feDataProc(bsOdr, gNdMat, gElm, gBdyD, gBdyN, gIntNd, gBdyDP, ...
	       gElcond, drFlag); 

%change the coordinates from mm to meters
Ndm(:,2:3) = Ndm(:,2:3);
...
Fg = sparse(Nukwn, 1);            % Fg: zeroth mode (mean) of the
				  % global RHS  vector
Fg_BC = sparse(Nukwn,1);          % Fg_BC: first mode (variance) of
                                  % the global RHS vector wrt
                                  % boundary conditions
...
Uvec_BC = sparse(Nukwn+NbdD,1);         % First moment (variance) of the
                                        % Unknowns +  Boundary Values
Uvec_BC(Nukwn+1:Nukwn+NbdD,1) = bcVar;  % set to constant  over all
                                        % BVs 
									
					

BdyDP_2 = Uvec_BC(Nukwn+1:Nukwn+NbdD,1);
...
for ie = 1:Nel % For each element,
  ...
  for s = 1:3
    ...
    % For each node (3-node in an element)
    
    % Coordinate Generation
    coord(s, 1) = Ndm(Elm(ie, s), 2); % x-coord   
    coord(s, 2) = Ndm(Elm(ie, s), 3); % y-coord
    
    % Conductivity Generation
    condc(s, 1) = Elcond(ie, 1); % 4 components of conductivity matrix
    condc(s, 4) = Elcond(ie, 1); % 4 components of conductivity matrix
  ...
  for iq = 1:NzR
    ...
    % Conductivity should obtained from 
    % 1) exact function type: 
    % Cond4 = feInConduct(ie, VecXYe(:,:,1), VecXYe(:,:,2));

    % 2) result of interpolation from known values on nodes 
  ...
  for row = 1:NlocDim
    ...
    if grow <= Nukwn
      ...
      Fg(grow, 1) = Fg(grow, 1) + LocSoMat(row, 1);
      Fg_BC(grow, 1) = 0;
      for col = 1:NlocDim
	...
	if gcol <= Nukwn
	  ...
	  Fg(grow, 1) = Fg(grow, 1) - LocElt * (Uvec(gcol, 1));
	  Fg_BC(grow, 1) = Fg_BC(grow, 1) - LocElt * Uvec_BC(gcol, ...
						  1);
	end

      end % for col
      
    end % for if grow<=Nukwn
...
if infoFlag == 1 
  TimeVecInfo = ['WRTINGDATA';'PREPROCDAT';'PREPARSTOR'; ...
		 'LOCBASISCF';'QDRTPOINTS';'ED_COMMELT'; ...
		 'LOCAL_MATX';'INVER_SION']  
  TimeVec
end
