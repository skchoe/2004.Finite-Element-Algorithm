% %<<<<<<< fePostProcOrdering.m
% function [nBdyNP, nBdyDP] = fePostProcOrdering(Ndm, NobN, NobD, BdyN, BdyD, SolBd, BdyDP, Elm)
% 
%   Nnd = size(Ndm, 1);
%   NbdN = size(BdyN, 1)
%   NbdD = size(BdyD, 1)
...
% 
%   
%   % Givn nodes(geometric/interpolant) fill NdEltCell with elements and order in each of them 
...
%   [nBdyN, nBdyNP] = subPostProcOrdering(NobN, Nnde, Odr, BdyN, SolBd, Elm, NdEltCell);
% 
%   figure(444);
%   subplot(2,1,1);
%     plot(SolBd);
...
%     
%     
%   [nBdyD, nBdyDP] = subPostProcOrdering(NobD, Nnde, Odr, BdyD, BdyDP, Elm, NdEltCell)
%   
%   figure(888);
%   subplot(2,1,1);
%     plot(BdyDP);
...
% 
% 
% function [nBdy, nBdyP] = subPostProcOrdering(Nob, Nnde, Odr, Bdy, Soln, Elm, NdEltCell)
% 
...
function [nBdyN, outerNodes] = fePostProcOrdering(Ndm, NobN, NobD, BdyN, ...
					       BdyD, SolBd, BdyDP, Elm) 
  ...
  % Given Element Set, find neighboring 
  [outerNodes, nBdyN] = subPostProcOrdering(NobN, Nnde, Odr, BdyN, SolBd, ...
					Elm, NdEltCell); 
  figure(444);
  subplot(2,1,1);
  plot(SolBd);
  ...
  %     
  %     
  %   [nBdyD, outerNodes] = subPostProcOrdering(NobD, Nnde, Odr, BdyD,
  %   BdyDP, Elm, NdEltCell) 
  %   
  %   figure(888);
  %   subplot(2,1,1);
